  ...
  return (
    <div>
      ...
      <div className='flex items-center gap-2'>
        <label>
            {data.image ? (
                <img src={typeof data.image ==='string' ? data.image : URL.createObjectURL(data.image)} alt="user-image" className='h-16 w-16 rounded-full object-cover mt-5 ring ring-slate-300 hover:opacity-80'/>
            ):(
            ...
            {fields.map((field)=>{
                ...
                <div key={field.key} className='mt-5 space-y-1 '>
                    ...
                    <input type={field.type} value={data[field.key] || ''} onChange={(e)=>handleChange(field.key,e.target.value)} required={field.required} className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm' placeholder={`Enter ${field.label.toLowerCase()}`}/>
                </div>
                )
