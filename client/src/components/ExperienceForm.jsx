...
const ExperienceForm = ({data,onChange}) => {
    ...
    const generateExperience=async(index)=>{
            ...
            try {
               const {data}=await api.post('/api/ai/enhance-pro-sum',{userContent:prompt},{headers:{Authorization:token }}) 
            ...
            {data.map((experience,index)=>(
                <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                    ...
                    <div className='space-y-2'>
                        ...
                        <button onClick={()=>generateExperience(index)} disabled={generatingIndex===index||!experience.position||!experience.company} className='flex items-center gap-1 px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors disabled:opacity-50'>
                            {generatingIndex===index ? <Loader2 className='w-3 h-3 animate-spin'/>:<Sparkles className='w-3 h-3'/>}
                            Enhance with AI
                        </button>
                        </div>
                        <textarea value={experience.description || ""} onChange={(e)=>updateExperience(index,"description",e.target.value)} className='w-full text-sm px-3 py-2 rounded-lg resize-none'placeholder="Describeyour key responsibilities."/>
                    </div>
