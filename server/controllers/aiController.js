...
//controller fro enhancing the resume's job description
...
export const uploadResume=async(req, res)=>{
    try{
        ...
        if(!resumeText){
            return res.status(400).json({message:"Resume text not found"})
        }

        const systemPrompt="you are an expert AI Agent to extract data from resume."
        const userPrompt=`Extract the following information from the resume text: ${resumeText}  
        
        provide data in the following JSON format with no aditional text before or after:
